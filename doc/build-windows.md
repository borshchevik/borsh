...
Below are some notes on how to build Borshchevik Core for Windows.

The options known to work for building Borshchevik Core on Windows are:
...
Note that for WSL the Borshchevik Core source path MUST be somewhere in the default mount file system, for
example /usr/src/borsh, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\borsh`, for example:

    make install DESTDIR=/mnt/c/workspace/borsh
