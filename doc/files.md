...
The data directory is the default location where the Borsh Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.borsh/`
macOS    | `$HOME/Library/Application Support/Borshchevik/`
Windows  | `%APPDATA%\Borshchevik\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `borsh.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Borsh blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `borsh.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `borshd` or `borsh-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `borshd.pid`        | Stores the process ID (PID) of `borshd` or `borsh-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `borshd` or `borsh-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [borsh.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`borsh-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Borshchevik Core:
